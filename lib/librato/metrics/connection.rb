    ...
    class Connection
      ...
      def transport
        ...
        @transport ||= Faraday::Connection.new(
          url: api_endpoint + '/v1/',
          request: { open_timeout: 20, timeout: 30 }
        ) do |f|
                         f.use Librato::Metrics::Middleware::ExpectsStatus

                         f.request :authorization, :basic, @client.email, @client.api_key
      ...
      def user_agent
        ...
        ua_chunks << agent_identifier if agent_identifier && !agent_identifier.empty?
      ...
      def transport_adapter
        transport.builder.handlers.each do |handler|
          return handler.name[18..-1] if handler.name[0, 16] == 'Faraday::Adapter'
        end
      end
    end
  end
